...
\begin{itemize}
    ...
    We will check whether the board is functional by, for instance, running the Blink example, which will blink the built-in LED[1]. 
    Other built-in examples can be run as well for further validations [2].
    \item \textit{Array of optical sensors:}
    We will test each optical sensor by connecting it to an analog input pin and printing its readings to the Serial Monitor using \texttt{analogRead()} [3]. 
    We will then see if there is a noticeable/sufficient change in the output value as we pull a punched card over the sensor.
    \item \textit{Low-power motor:} 
    We will verify the motor's functionality by connecting it to the Arduino through a motor driver and using the \texttt{analogWrite()} function to send various PWM signals to it [4]. 
    If the motor can operate/rotate at a reasonable rate, corresponding to the PWM values sent, then the motor can be deemed as working.
\end{itemize}
...
\hspace{1em} \hl{TODO(zzmic): Add references to the terminologies and components that we're going to use, especially for the \textbf{Hardware and Testing Plan} and \textbf{Preliminary List of Components to be Ordered} subsections.} \hl{TODO(zzmic): "Mimic" the behavior of LaTeX's bibliography management:} \url{https://www.overleaf.com/learn/latex/Bibliography_management_with_bibtex}
\begin{enumerate}[label={[\arabic*]}]
    \item https://docs.arduino.cc/built-in-examples/basics/Blink/
    \item https://docs.arduino.cc/built-in-examples/
    \item https://docs.arduino.cc/language-reference/en/functions/analog-io/analogRead/
    \item https://docs.arduino.cc/language-reference/en/functions/analog-io/analogWrite/
\end{enumerate}
